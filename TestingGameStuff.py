...
while PlayAgain:
    running = True
    while running:
        pygame.time.Clock().tick(700)  # locks game to 700fps so the program isn't vastly different on different computers
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
    ...
    while PlayingGame:
        pygame.time.Clock().tick(700)  # locks game to 60fps so the program isn't intensive on game over screen
        for event in pygame.event.get():
